...
module.exports = {
  async onPreBuild({ utils, constants }) {
    ...
    if (await utils.cache.has(lastLollyRef)) {
      ...
      console.log(
        chalk.green("Restored from build cache"),
        "- previous timestamp from cache"
      );
    } else {
  ...
  async onPostBuild({ utils, constants }) {
    ...
    console.log(
      chalk.green("Added to build cache"),
      `- latest lolly page timestamp: ${new_ts}`
    );
